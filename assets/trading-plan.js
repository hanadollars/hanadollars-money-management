...
(function () {
  ...
  function markStale() { if (planCard) planCard.classList.add("stale"); }
  function clearStale() { if (planCard) planCard.classList.remove("stale"); }

  /* Same locale-proof parsing as the sizer: a comma is a thousands separator
     in an amount, a decimal separator in a ratio or a percentage. */
  ...
  function nd(el) { return parseNum(el.value, true); }      /* ratios, percentages */
  ...
  function calc() {
    var msgs = [], bad = false;
    clearStale();
    var C = n(cap), TG = n(tgt), M = months(), R = nd(rr);
    var L = mode === "amt" ? n(amtIn) : NaN;
    var f = mode === "pct" ? nd(pctIn) / 100 : NaN;
  ...
  amtIn.addEventListener("input", function () { if (mode !== "amt") setMode("amt", true); markStale(); });
  pctIn.addEventListener("input", function () { if (mode !== "pct") setMode("pct", true); markStale(); });
  ...
  [cap, tgt, hor, rr].forEach(function (e) { e.addEventListener("input", markStale); });
  unit.addEventListener("change", calc);

  [cap, tgt, hor, rr, amtIn, pctIn].forEach(function (e) {
  ...
  $("pRun").addEventListener("click", calc);
