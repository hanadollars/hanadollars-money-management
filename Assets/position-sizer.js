...
(function () {
  ...
  function dec() {
    var s = String(entry.value), i = s.indexOf(".");
    var d = i < 0 ? 2 : Math.min(8, s.length - i - 1);
    return d < 2 ? 2 : d;
  }
  ...
  function levels() {
    var E = n(entry);
    if (slMode() === "price") return { E: E, S: n(sl), T: n(tp) };
  ...
  function expand(el) {
    ...
    var raw = String(el.value).trim(), E = n(entry);
  ...
  function applyMode() {
    var pts = slMode() === "points", ps = pointSize(), E = n(entry);
  ...
  riskPct.addEventListener("input", function () { anchor = "pct"; calc(); });
  riskUsd.addEventListener("input", function () { anchor = "amt"; calc(); });
  ...
  [sl, tp].forEach(function (e) {
    e.addEventListener("change", function () { expand(e); calc(); });
    e.addEventListener("keydown", function (ev) {
      if (ev.key === "Enter") { expand(e); calc(); e.blur(); }
    });
  });
